...
// modifications by Bob Gamble for use with Rpi Pico to init with myWire
//
...
//    Wire.setPins(iSDA, iSCL);

    // TOUCH_WIRE.setSDA(pinSda);
    // TOUCH_WIRE.setSCL(pinScl);
    // TOUCH_WIRE.begin(); // this is for generic MCUs

	myWire->setSDA(iSDA);
	myWire->setSCL(iSCL);
    myWire->begin(); // this is for generic MCUs
